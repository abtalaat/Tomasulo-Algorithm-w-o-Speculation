class MultiplierALU:
    ...
    def exe(self, number, op, v1, v2, dest):
        if op == "MUL" or op == "DIV":  # Check if it's a MUL or DIV operation and start the clock
            ...
            self.start_clock_mul[number] = self.system1.clock
            self.start_mul[number] = 1

        elif op == 0 and self.system1.clock == self.start_clock_mul[number] + self.system1.mul_time and self.start_clock_mul[number] != 0:  # Send the result on CDB after the "computing" time
            if self.op_mul[number] == "MUL":
                self.system1.result_queue.append([self.dest_mul[number], self.mul1[number] * self.mul2[number]])
